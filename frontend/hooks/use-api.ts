...
export function useApi() {
  ...
  const apiPatch = useCallback(async (endpoint: string, data: any) => {
    const response = await apiCall(endpoint, {
      method: 'PATCH',
      body: JSON.stringify(data)
    })
  ...
  return {
    ...
    getCustomForm: (id: number) => apiGet(API_ENDPOINTS.CUSTOM_FORMS.GET(id)),
    updateCustomForm: (id: number, data: any) => apiPut(API_ENDPOINTS.CUSTOM_FORMS.UPDATE(id), data),
    deleteCustomForm: (id: number) => apiDelete(API_ENDPOINTS.CUSTOM_FORMS.DELETE(id)),
    ...
    getFormByAccessToken: (token: string) => apiGet(API_ENDPOINTS.CUSTOM_FORMS.PUBLIC_ACCESS(token)),
    submitFormWithAccess: (data: any) => apiPost(API_ENDPOINTS.CUSTOM_FORMS.PUBLIC_SUBMIT, data),
    deactivateFormAccess: (accessId: number) => apiDelete(API_ENDPOINTS.CUSTOM_FORMS.DEACTIVATE_ACCESS(accessId)),
